...
const Community = () => {
    ...
    const fetchProjects = async() =>{
        try {
           ...
           // ✅ FIXED: Handle case where projects might be undefined or wrong property name
           setProjects(data.projects || []); // Changed from data.Projects to data.projects (lowercase)
           setLoading(false);
        } catch (error:any) {
            console.log(error);
            setLoading(false); // ✅ FIXED: Set loading to false even on error
                            ...
                            {projects.map((project)=>(
                                <Link key= {project.id} to={`/view/${project.id}`} target='_blank' className="w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden group hover:border-indigo-800/80 transition-all duration-300">
                                    ...
                                    <div className="relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800">
                                        ...
                                        // ✅ FIXED: Improved iframe sandbox for better security
                                        <iframe 
                                        ...
                                        {/*content*/}
                                    {/* ✅ FIXED: Removed invalid Tailwind class hover:bg-coral */}
